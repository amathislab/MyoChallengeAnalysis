...
def get_layers(config, envir, net, env_name, num_ep, render=False):
    ...
    for i in range(num_ep):
        ...
        while not done:
            ...
            mlp1_state = model.policy.mlp_extractor.policy_net[0](
                torch.Tensor(np.squeeze(lstm_states[0]))
            )
            mlp2_state = model.policy.mlp_extractor.policy_net[2](
                model.policy.mlp_extractor.policy_net[1](mlp1_state)
            )
            ...
            d = {
                ...
                "actions": action,
                "rewards": rewards,
                "LSTM cell state": np.squeeze(lstm_states[1]),
                "LSTM hidden state": np.squeeze(lstm_states[0]),
                "Linear layer 1": mlp1_state,
                "Linear layer 2": mlp2_state,
            }
...
def cross_project_kin(vel1, vel2, n_comp, n_highpc):
    ...
    pca_1 = pca.fit(vel1)
    return {
        "projection 1 on 2": pca_2.transform(vel1),
        "projection 2 on 1": pca_1.transform(vel2),
        "V1_1": np.cumsum(pca_1.explained_variance_ratio_)[n_highpc - 1],
        "V1_2": np.cumsum(pca_2.explained_variance_ratio_)[n_highpc - 1],
    }
