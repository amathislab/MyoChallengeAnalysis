...
if __name__=="__main__":
    ...
    for k in range(n_comp):
        ...
        projected_conds = [{'label':cond['encoding'], 'projected velocity':np.dot(cond['hand velocity'],components.T)} for cond in conds]
    ...
    fp = open('/home/ingster/Bureau/SIL-BigResults/class_performance_tasks', 'wb')
    pickle.dump(performance,fp)
    fp.close()
    
    pc_low_variance = next(x[0] for x in enumerate(pca.explained_variance_ratio_) if x[1] < 0.01)
    performance = pickle.load(open('/home/ingster/Bureau/SIL-BigResults/class_performance_tasks','rb'))
    plt.plot([n for n in range(n_comp)],performance,'-o',linewidth=1,markersize=2,color='black')
    plt.axvline(x=pc_low_variance, ymax=0.87,color='r', linestyle='-',linewidth=1,label='1% Variance')
    plt.legend()
    plt.xlabel('Number of PC removed')
    plt.ylabel('Accuracy of the linear classifier')
    plt.title('Classification performance : hold, cw, ccw')
    plt.savefig(os.path.join(ROOT_DIR,'SIL-Results/Motor-synergies/Low-level-PCs/tasks_classperf.png'))
